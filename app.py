...
/* POST-ITS CLICÁVEIS PARA VOTAÇÃO */
.vote-card-wrapper .stButton > button {
...
.vote-card-wrapper .stButton > button:hover {
    background: transparent !important;
    border: 0 !important;
    color: inherit !important;
    box-shadow: none !important;
...
def dica_voto_html(votado=False):
    ...
    if votado:
        ...
        return (
            '<div class="vote-hint-voted">'
            '✓ Voto registrado — toque novamente para desfazer'
            '</div>'
            if st.session_state.get("idioma", "pt") == "pt"
            else
            '<div class="vote-hint-voted">'
            '✓ Vote registered — tap again to undo'
            '</div>'
        )

    return (
        '<div class="vote-hint">Toque no post-it para votar</div>'
        if st.session_state.get("idioma", "pt") == "pt"
        else
    ...
    if mostrar_anterior:
        ...
        with col_nav_1:
            ...
            if st.button(
                ...
                st.session_state.aba_pendente = abas_ordem[novo_indice]
                st.rerun()
...
if "aba_atual_valor" not in st.session_state:
    st.session_state.aba_atual_valor = abas_ordem[0]

if "aba_radio_versao" not in st.session_state:
    st.session_state.aba_radio_versao = 0

if "aba_pendente" in st.session_state:
...
indice_padrao_aba = abas_ordem.index(st.session_state.aba_atual_valor)

aba_atual = st.radio(
    "Navegação",
    abas_ordem,
    index=indice_padrao_aba,
    horizontal=True,
    label_visibility="collapsed",
    key=f"aba_radio_{st.session_state.aba_radio_versao}"
)
    ...
    if postits:
        ...
        if st.session_state.get("is_mobile", False):
            ...
            for postit_id, equipe, texto, votos in postits:
                ...
                foi_votado = postit_id in st.session_state.postits_votados

                html_card_voto = f"""
<div class="{classe_postit} postit-clickable">
<h4>{esc(equipe)}</h4>
<div class="postit-texto">{esc(texto)}</div>
<div class="votos">{t("votes")}: {votos}</div>
{dica_voto_html(foi_votado)}
</div>
"""
            ...
            for i, (postit_id, equipe, texto, votos) in enumerate(postits):
                ...
                with colunas_postits[i % len(colunas_postits)]:
                    ...
                    html_card_voto = f"""
<div class="desktop-card-html {classe_postit} postit-clickable" style="
    display:flex;
    flex-direction:column;
    justify-content:space-between;
    min-height:260px;
...
"""
                    ...
                    html_postit = html_postit.replace(
                        '<div class="desktop-card-html"',
                        '<div class="desktop-card-html postit-clickable"'
                    ).replace(
                        "</div>\n",
                        f"{dica_voto_html(foi_votado)}</div>\n",
                        1
                    )
            ...
            if hipoteses_categoria:
                ...
                for i, (hipotese_id, texto_hipotese, votos_hipotese) in enumerate(hipoteses_categoria):
                    ...
                    with colunas_hipoteses[i % len(colunas_hipoteses)]:
                        ...
                        html_card_voto = f"""
<div class="desktop-card-html {classe_solucao} postit-clickable" style="
    display:flex;
    flex-direction:column;
    justify-content:space-between;
    min-height:240px;
